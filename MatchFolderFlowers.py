...
def saveTrainingFileNames(img_list):
    ...
    with open('json/selected_flowers_training_images.json', 'w', encoding='utf8') as outfile:
...
def saveValidationFileNames(img_list):
    ...
    # write to the json file with the updated dictionary
    with open('json/selected_flowers_validation_images.json', 'w', encoding='utf8') as outfile:
        str_ = json.dumps(cleanDictionary(upd_dictionary),
                          indent=4, sort_keys=True,
                          separators=(',', ': '), ensure_ascii=False)
...
def deleteTrainingFileContents():
    ...
    with open('json/selected_flowers_training_images.json', 'w', encoding='utf-8') as json_file:
        json_file.truncate()
        json_file.close()
...
def imageProcessing(query_image, training_image, imageName, xx, label, lbl_id):
    ...
    # Apply ratio test
    lowest_ratio = 0.8  # the best value given by Lowe(2004)
...
def sortTrainImages():
    ...
    for lbl in train_labels:
        ...
        my_path = './Data3/former_train/' + lbl  # images from the imagenet source
...
def getSelectedTrainingImages():
    with open('json/selected_flowers_training_images.json') as selected_images_file:
        ...
        df.to_csv('csv/flowers_train_data.csv', index=False)
