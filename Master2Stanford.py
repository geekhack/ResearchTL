#import MatchFolderStandford as mt
import pandas as pd
import numpy as np
...
# t_model.add(layers.Dropout(0.5))
# t_model.add(layers.BatchNormalization())
t_model.add(layers.Dense(9, activation='softmax',kernel_regularizer=l2(0.01), bias_regularizer=l2(0.01)))

t_model.compile(loss=losses.CategoricalCrossentropy(from_logits=True),
                optimizer=optimizers.Adam(lr=0.001),
                metrics=['accuracy'])
clr_triangular = CyclicLR(mode='triangular2')
history = t_model.fit(training_dataset, batch_size=4, epochs=50, verbose=1)
...
t_model.evaluate(validation_dataset, batch_size=4, verbose=2)
...
# load the image
images_array = ['00650.jpg', '00675.jpg'];
