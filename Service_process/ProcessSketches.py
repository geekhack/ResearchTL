...
def saveTrainingFileNames(img_list):
    ...
    with open('../json/skew_sketches_images.json', 'w', encoding='utf8') as outfile:
...
def deleteTrainingFileContents():
    ...
    with open('../json/skew_sketches_images.json', 'w', encoding='utf-8') as json_file:
        json_file.truncate()
        json_file.close()
...
def sortTrainImages():
    ...
    for lbl in train_labels:
        ...
        my_path = '../Data5/cloud/' + lbl  # images from the imagenet source
...
def getSelectedTrainingImages():
    with open('../json/skew_sketches_images.json') as selected_images_file:
        ...
        df.to_csv('../csv/skew_sketches_data.csv', index=False)
