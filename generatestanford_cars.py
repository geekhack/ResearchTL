...
mat_train = loadmat('Data2/stanfordcars/cars_train_annos.mat')
mat_test = loadmat('Data2/stanfordcars/cars_test_annos.mat')
meta = loadmat('Data2/stanfordcars/cars_meta.mat')
...
# get the length of the classes
cars_classes_length = len(labels)

#create two csvs to generate the datasets(training and testing)
df_training = pd.DataFrame(train)
df_training.columns = ['file', 'label']
df_training.to_csv('stanford_data.csv', index=False)
...
train_df = pd.read_csv("stanford_data.csv")
...
training_dataset = train_dataset_gen.flow_from_dataframe(dataframe=train_df,
                                                         directory="./Data2/stanfordcars/cars_train/",
                                                         x_col="file",
                                                         y_col="label",
                                                         shuffle=True,
...
layer_test_higher= [139,143,146,149,150,155,158,161,165,168,171]
...
t_model.add(layers.Dropout(0.5))
t_model.add(layers.BatchNormalization())
t_model.add(layers.Dense(16, activation='relu'))
t_model.add(layers.Dropout(0.5))
t_model.add(layers.BatchNormalization())
t_model.add(layers.Dense(184, activation='softmax'))
steps_per_epoch = round(int(len(training_dataset.filenames))/int(BATCH_SIZE))

#set the learning_rate schedule to reduce it over time
...
t_model.compile(loss=losses.CategoricalCrossentropy(from_logits=True),
              ...
              metrics=['accuracy'])
history = t_model.fit(training_dataset, batch_size=BATCH_SIZE, epochs=10, verbose=1)
...
# load the image
images_array=['00650.jpg','00675.jpg','00677.jpg'];
